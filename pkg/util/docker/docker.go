...
func Inspect(id string, withSize bool) (types.ContainerJSON, error) {
	...
	// cache the inspect for 5 minutes to reduce pressure on the daemon
	cache.Cache.Set(cacheKey, container, 5*time.Minute)
...
// GetPsCacheKey returns the key to the container list in the agent cache
func (cfg *ContainerListConfig) GetPsCacheKey() string {
