...
This repository contains automated integration tests for the Datadog Agent. It
...
Non-bundled dependencies:
 - [VirtualBox](https://www.virtualbox.org) (last tested with 4.3.16)
 - [Vagrant](http://www.vagrantup.com/) (last tested with
   [1.7.2](http://downloads.vagrantup.com/tags/v1.7.2))
...
- To execute the tests on Azure, you need Azure credentials in the environment. You will need to set the following: AZURE_CLIENT_ID AZURE_CLIENT_SECRET AZURE_TENANT_ID AZURE_SUBSCRIPTION_ID.
- you can then execute `bash tasks/run-test-kitchen.sh`.
...
* Not all supported operating systems are tested. Missing operating systems
  include:
    ...
    - Amazon Linux
    - SmartOS
* The source install and the source install script are currently not tested
* Memory leaks or any obscurities that result from the Agent running for a long time
...
### Adding a Platform
Tests are executed either on Vagrant virtual machines, which are spun up from Vagrant
boxes, or on Digital Ocean instances. Platforms are defined in `.kitchen.yml`.

The [Bento](https://github.com/chef/bento) project has pre-built Vagrant boxes for
several common operating systems. If you don't find a suitable box there, you can also
make your own using [Veewee](https://github.com/jedi4ever/veewee).
